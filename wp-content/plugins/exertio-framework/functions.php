		...
		<div class="wr-ap">
			...
			<div id="welcome-panel" class="welcome-panel">
				...
				<div class="welcome-panel-contents">
					<h2 style="display: inline;"><?php esc_html_e("Exertio - Freelance Marketplace WordPress Theme ", "exertio_theme");?>
					</h2>
					<a  href="<?php echo get_admin_url(). "?dismis_welcome=1"?>" aria-label="Dismiss the welcome panel" style="color: white;font-size: 20px;">Dismiss</a>
				</div>
...
if(!function_exists('exertio_database_update_notice_column'))
{
    function exertio_database_update_notice_column(){
        $exists_value = get_option( '_notification_table_updated' );
        global $wpdb;
        ...
        if (empty($wpdb->num_rows)) {
            echo '<div class="notice notice-error exertio-notic">
					<h1> ' . __("Exertio WordPress Theme database update needed .", "exertio_theme") . '</h1>
				 <p>' . __("To use <b>Exertio WordPress Theme</b> properly we must need to update database for the new features and fixes. Please click on button below to update it. ", "exertio_theme") . '</p>
				 <button class="button button-primary" id="update_database_column">' . __("Update Now", "exertio_theme") . '</button>
			 </div>';
        }
    }
    add_action('admin_notices', 'exertio_database_update_notice_column');
}
