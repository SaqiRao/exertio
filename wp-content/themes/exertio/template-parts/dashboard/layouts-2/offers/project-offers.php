      ...
      <div class="d-flex justify-content-between flex-wrap">
        <div class="d-flex align-items-end flex-wrap">
          <div class="mr-md-3 mr-xl-5">
            <h2><?php echo esc_html__('Project Offers','exertio_theme'); ?></h2>
      ...
      <div class="card mb-4">
        <div class="card-body">
          <div class="pro-section">
				  ...
				  <div class="pro-coulmn">
					  <?php 
							...
							else if($type == 'hourly' || $type == 2)
							{
								...
								echo '<small class="estimated-hours">'.esc_html__( 'Estimated Hours ', 'exertio_theme' ).$result->allowed_hours.'</small>';
							}
						 ?>
				  </div>
				  <div class="pro-coulmn">
				  	<?php
                        if($result->offer_Status == "1") {
                           $offer_Status   =   esc_html__('Pending','exertio_theme');
                        }
                         else if($result->offer_Status == "2"){
                         $offer_Status   =   esc_html__('Accepted','exertio_theme');
                         }

                         else if($result->offer_Status == "3"){
                         	$offer_Status   =   esc_html__('Rejected','exertio_theme');
                         }
                         else if($result->offer_Status == "4"){
                         	$offer_Status   =   esc_html__('Cancelled By author','exertio_theme');
                         }
				  	 ?>
            ...
            <?php if($result->offer_Status != "2" && $result->offer_Status != "4" ){ ?>
						  <a href="javascript:void(0)" class="btn btn-inverse-primary btn-sm change-offer-status btn-loading" data-status ="accept" data-pid="<?php echo esc_attr($result->project_id ); ?>"  data-offer-id ="<?php  echo $result->id ?>" > <i class="far fa-edit"></i> <?php echo esc_html__( 'Accept', 'exertio_theme' ); ?><span class="bubbles"> <i class="fa fa-circle" aria-hidden="true"></i> <i class="fa fa-circle" aria-hidden="true"></i> <i class="fa fa-circle" aria-hidden="true"></i> </span></a>
            <?php  } ?>
	
              <?php if($result->offer_Status != "3" && $result->offer_Status != "4" ){ ?>
						  <a href="javascript:void(0)" class="btn btn-inverse-danger btn-sm change-offer-status" data-status ="reject" data-pid="<?php echo esc_attr($result->project_id ); ?>" data-status="remove"  data-offer-id ="<?php echo $result->id ?>"> <i class="far fa-times-circle"></i> <?php echo esc_html__( 'Reject', 'exertio_theme' ); ?></a>
                     <?php  } ?>
					  </span> 
				  </div>
				</div>
