              ...
              <div class="card">
                <div class="card-body">
                  ...
                  <div class="form-group col-md-6">
                    ...
                    <input type="number" class="form-control" name="fl_cell_number" value="<?php echo esc_attr(get_user_meta(  $current_user_id, 'user_cell_number' , true )); ?>">
                  </div>
                  <?php $uid = get_current_user_id(); 
                         $user_provider_type = get_user_meta($uid, 'user_provider_type', true);
                      ?>
                    <div class="form-group col-md-6">
                      <label><?php echo esc_html__('Provider Type','exertio_theme'); ?></label>
                    <select id="providerType" name="providerType" class="form-control provider-type">
                      <option value=""><?php echo __('Provider Type'); ?></option>
                      <option value="non-medical" <?php selected($user_provider_type, 'non-medical'); ?>><?php echo __('Non-Medical'); ?></option>
                      <option value="nurse" <?php selected($user_provider_type, 'nurse'); ?>>Nurse</option>
                      <option value="mid-level-practitioner" <?php selected($user_provider_type, 'mid-level-practitioner'); ?>><?php echo __('Mid-level Practitioner'); ?></option>
                      <option value="medical-physician" <?php selected($user_provider_type, 'medical-physician'); ?>><?php echo __('Medical Physician'); ?></option>
                      <option value="other-medical-provider" <?php selected($user_provider_type, 'other-medical-provider'); ?>><?php echo __('Other Medical Provider'); ?></option>
                    </select>
                    </div>

                     <div class="form-group col-md-6">
                              <label><?php echo esc_html__('Country','exertio_theme'); ?></label>
                              <?php
                              ...
                              $location_taxonomies = exertio_get_terms('freelancer-locations');
                                ...
                                {
                                    echo '<select name="freelancer_locations" class="form-control general_select" '.$custom_location_check.'>'.get_hierarchical_terms_for_user('freelancer-locations', 'user_freelancer_countries', $uid ).'</select>';
                                }
                            ?>
                      </div>
...
if (!function_exists('get_hierarchical_terms_for_user')) {
    function get_hierarchical_terms_for_user($taxonomy_name = '', $user_meta_key = '', $uid = '') {
        // Fetch the user meta value
        $selected_ids = get_user_meta($uid, $user_meta_key, true);
        ...
        $hierarchy = '';
        $taxonomies = exertio_get_terms($taxonomy_name);
        ...
        foreach ($taxonomies as $term) {
            ...
            $selected = ($term->term_id == $selected_ids) ? 'selected="selected"' : '';
            ...
            if (isset($hierarchy[$term->term_id])) {
                ...
                foreach ($hierarchy[$term->term_id] as $child) {
                    ...
                    $selected = ($child->term_id == $selected_ids) ? 'selected="selected"' : '';
                    ...
                    if (isset($hierarchy[$child->term_id])) {
                        foreach ($hierarchy[$child->term_id] as $child2) {
                            // Get the term object by its ID
                            $child2 = get_term($child2, $taxonomy_name);
                            $selected = ($child2->term_id == $selected_ids) ? 'selected="selected"' : '';
                            ...
                            if (isset($hierarchy[$child2->term_id])) {
                                foreach ($hierarchy[$child2->term_id] as $child3) {
                                    // Get the term object by its ID
                                    $child3 = get_term($child3, $taxonomy_name);
                                    $selected = ($child3->term_id == $selected_ids) ? 'selected="selected"' : '';
